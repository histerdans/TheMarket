...
from JiuzeSite.utils import random_string_generator, unique_key_generator
#send_mail(subject, message, from_email, recipient_list, html_message)

DEFAULT_ACTIVATION_DAYS = getattr(settings, 'DEFAULT_ACTIVATION_DAYS', 7)

class UserManager(BaseUserManager):
    def create_user(self, email, full_name=None, password=None, is_pending =False, is_active=True, is_staff=True, is_admin=False):
        if not email:
...
class User(AbstractBaseUser):
    ...
    pending   = models.BooleanField(default=False) # have inactive sessions
...
class UserToken(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    customer_id       = models.CharField(max_length=120, blank=True) # AB31DE3
    client_id      = models.CharField(max_length=120, blank=True) # AB31DE3
